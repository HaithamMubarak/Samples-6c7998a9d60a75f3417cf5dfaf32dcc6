...
/**
 * ArrayBufferStream : simple stream with arraybuffer data,
 *  you can either append arraybuffer,blob or array of unsigned bytes.
 * 
 * ArrayBufferStream is initialized by maximum number of bytes to store, 
 * when the append operation lead to overflow, it will prevent append oepration and 
 * set the this.almostFull to true, indicates the buffer should be read.
 * 
 * 
 */
(function() {
	...
	var ArrayBufferStream = function(capacity) {
		...
		if (this.constructor != ArrayBufferStream) {
			throw new Error(
					'ArrayBufferStream cannot be called as a function, use new operator')
		}
		...
		/**
		 * Clone the current object and returns a new object with cloned buffer
		 */
		this.clone = function() {
		...
		this.append = function(object) {
			...
			if (object instanceof ArrayBuffer) {
				...
				if (arraybuffer.byteLength > this.free()) {
					...
					throw new Error(
							'append operation failed, cannot add new arraybuffer with size '
									+ arraybuffer.byteLength + ', only '
									+ this.free() + ' bytes are free');
				}
				...
				if (array.length > this.free()) {
					this.almostFull = true;
					throw new Error(
							'append operation failed, cannot add new array with size '
									+ array.length + ', only ' + this.free()
									+ ' bytes are free');
				}
				...
				for (var i = 0; i < array.length; i++) {
					...
					if (!valid) {
						throw new Error(
								'Expected an array of unsigned bytes , invalid value '
										+ JSON.stringify(item) + ' at index '
										+ i)
					}
				}
				...
				console.log(object);
				throw new Error(
						'Invalid type, expected ArrayBuffer,Blob or array of bytes')
			}
		}
		...
		/**
		 * returns the available number of data stored i nthe buffer
		 */
